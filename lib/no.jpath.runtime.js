...
function JNode(data, root) {
    this.data = data;
    this.root = root || null;
};

JNode.root = function(data) {
    var root = new JNode(data);
    root.root = root;
...
JNode.prototype.nametest = function(name, result) {
    ...
    if ( Array.isArray(data) ) {
        ...
        for (var i = 0; i < data.length; i++) {
            ( new JNode( data[i], root ) ).nametest(name, result);
        }
        return result;
    }

    var node = new JNode(r, this.root);
    return node;
    /*
    if (result) {
