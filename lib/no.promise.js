...
//  Создаем из массива/объекта promise'ов новый promise, который зарезолвится только после того,
//  как зарезолвятся все promise'ы из массива/объекта. Результатом будет массив/объект результатов.
//
no.Promise.wait = function(promises) {
    if ( Array.isArray(promises) ) {
        return wait_array(promises);
    } else {
        return wait_object(promises);
    }
};
