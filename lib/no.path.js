...
no.Path._getter = function(path) {
    ...
    if (!compiled) {
        ...
        //  Если path начинается на [...], то parts[0] === ''.
        if (!parts[0]) { parts.shift(); }

        for (var i = 0, l = parts.length; i < l; i++) {
            var part = parts[i];

            //  Строка вида obj[3] или obj['foo'].
            var value = (/^[0-9]/.test(part)) ? 'obj[' + part + ']' : 'obj["' + part + '"]';
...
no.Path._setter = function(path) {
    ...
    if (!compiled) {
        ...
        //
        var l = parts.length;
        //  Обрабатываем все компоненты, кроме последней.
        for (var i = 1; i < l; i++) {
            var part = parts[i - 1];
            var next = parts[i];
            ...
            if (!part) {
                //  Добавляем вычисление длинны текущего массива.
                body += 'l = obj.length;';
            }
    ...
    function getValue(part) {
        if (!part) {
            //  Индекс не задан. Добавляем в конец массива.
            return 'obj[l]';
        }
        //  Строка вида obj[3] или obj['foo'].
        return (/^[0-9]/.test(part)) ? 'obj[' + part + ']' : 'obj["' + part + '"]';
    }
