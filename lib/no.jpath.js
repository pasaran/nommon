...
no.jpath.compile = function( expr, id, scheme ) {
    ...
    if ( !compiled ) {
        ...
        var js = 'var as=R.as,ss=R.ss,ass=R.ass,f=R.f,c=R.c,ts=R.ts,e=R.e;\n';
    ...
    for ( var i = 0, l = steps.length; i < l; i++ ) {
        ...
        switch ( id ) {
            ...
            case 'pred':
                ...
                } else if ( expr._scheme.is_boolean() ) {
                    //  Это гвард.
...
function ast2js( ast, d_scheme, r_scheme, exprs ) {
    ...
    switch ( ast._id ) {
        ...
        case 'binop':
            var op = ast.op;
            var left_js = ast2js( ast.left, d_scheme, r_scheme, exprs);
            var right_js = ast2js( ast.right, d_scheme, r_scheme, exprs );
