...
function cmpN(node, nodeset) {
    if (nodeset instanceof JNode) {
        ...
        var data = nodeset.data;
        if ( Array.isArray(data) ) {
            for (var i = 0; i < data.length; i++) {
                if ( value == ( new JNode( data[i] ) ).scalar() ) {
                    return true;
                }
            }
            return false;
...
function compileObject(obj) {
    ...
    return function(data, root, vars) {
        ...
        for (var key in items) {
            var t = items[key](data, root, vars);
            if (t instanceof JNode) {
                r[key] = t.data;
            } else if (t instanceof JNodeset) {
                r[key] = t.toArray();
            } else {
                r[key] = t;
            }
