...
no.Scheme.Default = no.Scheme.extend( {
    ...
    gen_index: function( exid ) {
        return 'if(a){d=d[e' + exid + '(d,r,v)];if(d==null)return;a=Array.isArray(d)}else{return}';
    },
...
no.jpath = function( expr, data, vars ) {
    return no.jpath.compile( expr, 'expr' )( data, data, vars );
};

no.jpath.string = function( expr ) {
    return no.jpath.compile( expr, 'string_content' );
};
...
no.jpath.compile = function( expr, id, scheme ) {
    var key = expr + '::' + id;
    ...
    if ( !compiled ) {
        ...
        var js = 'var as=R.as,ss=R.ss,ass=R.ass,f=R.f,c=R.c,ts=R.ts,tn=R.tn,tc=R.tc,e=R.e,M=R.M;\n';
...
function ast2js( ast, d_scheme, r_scheme, exprs ) {
    ...
    switch ( ast._id ) {
        ...
        case 'unop':
            ...
            ast._scheme = ( op === '-' ) ? no.Scheme.number : no.Scheme.boolean;
        ...
        case 'binop':
            ...
            if ( op === '~~' ) {
                ...
                ast._scheme = no.Scheme.boolean;

            } else if ( op === '&&' || op === '||' ) {
                var left_scheme = left._scheme;
                var right_scheme = right._scheme;

                ast._scheme = ( left_scheme === right_scheme ) ? left_scheme : no.Scheme.default;
...
no.jpath.defunc = function( name, def ) {
    ...
    if ( typeof def === 'function' ) {
        ...
        scheme = _schemes[ def.scheme ] || no.Scheme.default;
        body = def.body;
    }
