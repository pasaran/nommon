    ...
    for (var i = 0, l = steps.length; i < l; i++) {
        ...
        switch (id) {
            ...
            case 'pred':
                ...
                r += 'node = node.filter(function(node) { return (' + ast2js(step.expr) + '); });';
                break;

            case 'index':
                //  Cast `expr` to scalar.
                step.expr._as = TYPE_SCALAR;
                r += 'node = node.index(' ast2js(step.expr) + ');';
                break;

            case 'guard':
                r += 'if (!(' + ast2js(step.expr) + ')) { return empty; }';
                break;
            }
        }

        if (id !== 'guard') {
            r += 'if (node.isEmpty()) { return empty; }';
        }
    }
