...
no.jpath = function(expr, data, vars) {
    var compiled = no.jpath.compileExpr(expr);

    return compiled(data, data, vars);
};

no.jpath.compile = function(str, id) {
    var cached = _cache[str];

    if (!cached) {
        //  expr isn't cached.
        cached = _cache[str] = ast2func( parser.start(str, id || 'expr') );
    }
...
//
no.jpath.compileExpr = function(expr) {
...
no.jpath.compileString = function(str) {
    //  FIXME: Нужно эскейпить кавычки.
    var compiled = compileString('"' + str + '"');
...
function compileString(str) {
    ...
    if (!cached) {
        //  expr isn't cached.
        cached = _cache[str] = ast2func( parser.start(str, 'expr') );
    }
...
function compileObject(obj) {
    ...
    for (var key in obj) {
        items[key] = no.jpath.compileExpr( obj[key] );
    }
...
function compileArray(arr) {
    ...
    var l = arr.length;
    for (var i = 0; i < l; i++) {
        items.push( no.jpath.compileExpr( arr[i] ) );
    }
...
//
no.jpath._run = function(jid, data, root, vars) {
    ...
    for (var i = 0, l = jpath.length; i < l; i += 2) {
        ...
        switch ( jpath[i] ) {
            ...
            case 5:
                ...
                //  NOTE: slice/splice медленнее, чем ручное вырезание одного элемента.
                //  http://jsperf.com/slice-vs-splice-vs-index
                var r = current[index];
...
//  nodeset -> boolean
//
no.jpath.nodeset2boolean = function(nodeset) {
    if (nodeset) {
        if ( isArray(nodeset) ) {
            return (nodeset.length > 0) ? !!nodeset[0] : false;
        }
        return !!nodeset;
    }
...
//  nodeset -> scalar
//
no.jpath.nodeset2scalar = function(nodeset) {
    if ( isArray(nodeset) ) {
        return (nodeset.length) ? nodeValue( nodeset[0] ) : '';
    }
    return nodeValue(nodeset);
};

//  scalar -> boolean
//
no.jpath.scalar2boolean = function(scalar) {
    return !!scalar;
};
...
function ast2key(ast) {
    var start = ast._start;

    return ast._input
        .substr(start, ast._end - start)
        .replace(/\s*$/, '');
}
...
function ast2js(ast) {
    ...
    function jpath2js(jid, data) {
        return 'no.jpath._run("' + jid + '", ' + data + ', root, vars)';
    }
