    ...
    function r_expr() {
        ...
        function reduce(p) {
            ...
            while ( (( op = ops[0] )) && (BINOPS[op] > p) ) {
                ...
                var left = args.shift();
                args.unshift( wrap(left, ops.shift(), right) );
            }
        }

        function wrap(left, op, right) {
            var type = TYPE_BOOL;
            switch (op) {
                case '+':
                case '-':
...
function ast2js(ast) {
        ...
        case 'binop':
            ...
            if (op === '==' || op === '!=') {
                if (lt === TYPE_NODESET || rt === TYPE_NODESET) {
                    if (lt === rt) {
                        js = cmp('NN', ljs, rjs);
                    } else if (lt === TYPE_SCALAR) {
                        js = cmp('SN', ljs, rjs);
                    } else if (rt === TYPE_SCALAR) {
                        js = cmp('SN', rjs, ljs);
                    }
                }
                if (js && op === '!=') {
    ...
    function cmp(type, left, right) {
        return 'no.jpath._cmp' + type + '(' + left + ', ' + right + ')';
    }
}
