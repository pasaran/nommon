...
function compileString(expr, id) {
    ...
    if (!cached) {
        //  expr isn't cached.
        cached = _cache[expr] = compile( parser.start(expr, id || 'expr') );
    }
...
function compile(ast) {
    ...
    var p = ast2func(ast, exprs);

    var r = '';
    for (var i = 0; i <= p; i++) {
        r += 'function t' + i + '(node, root, vars) { ' + exprs[i] + ' }';
    }
    r += 'return function(data, vars) { var node = new JNode(data); return t' + p + '(node, node, vars); }';
...
function ast2func(ast, exprs) {
    var js = (ast._id === 'jpath') ? jpath2func(ast, exprs) : expr2func(ast, exprs);
    ...
    for (var i = 0, l = steps.length; i < l; i++) {
        ...
        switch (id) {
            ...
            case 'index':
                ...
                var p = ast2func(step.expr, exprs);
                r += 'node = node.filter(t' + p + ', root, vars);\n'
                break;
...
function ast2js(ast, exprs) {
    ...
    //  Typecasting.
    //  FIXME: scalar приводить к boolean.
