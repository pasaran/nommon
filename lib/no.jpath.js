...
no.jpath = function(expr, data, vars) {
    return unwrap( compileExpr(expr)(data, vars) );

};

no.jpath.raw = function(expr, data, vars) {
    return compileExpr(expr)(data, vars);
};
...
function compileObject(obj) {
    ...
    return function(data, vars) {
        ...
        for (var key in items) {
            r[key] = unwrap( items[key](data, vars) );
        }
...
function compileArray(arr) {
    ...
    return function(data, vars) {
        ...
        for (var i = 0; i < l; i++) {
            r.push( unwrap( items[i](data, vars) ) );
        }
