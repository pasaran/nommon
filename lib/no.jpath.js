...
function compile(ast) {
    ...
    for (var i = 0; i < exprs.length; i++) {
        r += 'function t' + i + '(node, root) { return (' + ast2js( exprs[i] ) + ') }';
    }
    r += 'return function(data) { var node = new JNode(data); return t0(node, node) }';

    return Function(r)();
}

function jpath2js(ast, exprs) {
    var r = '';
    ...
    for (var i = 0, l = steps.length; i < l; i++) {
        ...
        switch (id) {
            ...
            case 'pred':
                ...
                r += 'node = node.filter(t' + exprs.length + ', root);'
                exprs.push(expr);
                break;
            ...
            case 'index':
                ...
                r += 'node = node.index(t' + exprs.length + ', root);';
                exprs.push(expr);
                break;
