    ...
    for ( var i = 0, l = steps.length - 1; i <= l; i++ ) {
        ...
        switch ( step._id ) {
            case 'namestep':
                var name = step.name;
                js += ( is_last ) ?
                    'return a?R.as(d,"' + name + '",[]):d["' + name + '"]' :
                    'if(a){d=R.as(d,"' + name + '",[])}else{d=d["' + name + '"];if(d==null)return;a=Array.isArray(d)}';

                break;

            case 'starstep':
                js += ( is_last ) ?
                    'return (a?R.ass:R.ss)(d,[])' :
                    'd=(a?R.ass:R.ss)(d,[]);a=1;';
            ...
            case 'pred':
                ...
                js += ( is_last ) ?
                    'if(a)return R.f(d,r,v,e' + exid + ',[]);if(e' + exid + '(d,r,v))return d' :
                    'if(a){d=R.f(d,r,v,e' + exid + ',[])}else{if(!e' + exid + '(d,r,v))return}';
